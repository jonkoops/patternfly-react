...
export interface ClipboardCopyProps extends Omit<HTMLProps<HTMLDivElement>, 'onChange' | 'children'>, OUIAProps {
  ...
  onChange?: (event: FormEvent, text?: string) => void;
...
class ClipboardCopy extends Component<ClipboardCopyProps, ClipboardCopyState> {
  ...
  updateText = (event: FormEvent, text: string) => {
    this.setState({ text });
    this.props.onChange(event, text);
  };

  updateTextWhenExpanded = (event: FormEvent, text: string) => {
